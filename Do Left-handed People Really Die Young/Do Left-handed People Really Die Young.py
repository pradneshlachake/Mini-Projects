...
lefthanded_data['Birth_year'] = lefthanded_data['Age'].apply(lambda x: 1986-x)
...
ax.set_ylabel('Population') # set the y label for the plot
...
def P_lh(death_distribution_data, study_year = 1990): # sum over P_lh for each age group
    ...
    p_list = P_lh_given_A(death_distribution_data['Age'], study_year)* death_distribution_data['Both Sexes'].values # multiply number of dead people by P_lh_given_A
...
def P_A_given_lh(ages_of_death, death_distribution_data, study_year = 1990):
    ...
    P_A = np.concatenate([death_distribution_data[death_distribution_data.Age==age]['Both Sexes'].values/death_distribution_data['Both Sexes'].sum() for age in list(ages_of_death)])
    P_left = P_lh(death_distribution_data, study_year) # use P_lh function to get probability of left-handedness overall
    P_lh_A = P_lh_given_A(ages_of_death, study_year) # use P_lh_given_A to get probability of left-handedness for a certain age
    return pd.Series(P_lh_A*P_A/P_left)
...
def P_A_given_rh(ages_of_death, death_distribution_data, study_year = 1990):
    ...
    return pd.Series(P_rh_A*P_A/P_right)
